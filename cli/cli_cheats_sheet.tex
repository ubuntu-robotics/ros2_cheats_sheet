...
\cfoot{\copyright~2024 Canonical} % Actual footer text
...
%
\begin{tabularx}{\linewidth}{lX}
\smallhspace \cliverb{convert}      & Write out a new bag with different settings.\\
\smallhspace \cliverb{info}      & Output information of a bag.\\
\smallhspace \cliverb{list}      & Output list of available plugins.\\
\smallhspace \cliverb{play}      & Play a bag.\\
\smallhspace \cliverb{record}    & Record a bag.\\
\smallhspace \cliverb{reindex}    & Reconstruct metadata file for a bag.
\end{tabularx}
%
\\
Examples:
\\
\terminal{ros2 bag convert \ddash input \textangles{bag-name} storage\_id
\terminalinebreak~ \ddash output-options \textangles{settings-file-name}}\\
\terminal{ros2 bag info \textangles{bag-name}}\\
\terminal{ros2 bag play \textangles{bag-name}}\\
\terminal{ros2 bag record -a}\\
\terminal{ros2 bag reindex \textangles{bag-dir}}
%
...
%
\begin{tabularx}{\linewidth}{lX}
\smallhspace \cliverb{create\_enclave}        & Create enclave.                              \\
\smallhspace \cliverb{create\_keystore}       & Create keystore.                             \\
\smallhspace \cliverb{create\_permission}     & Create permission.                           \\
\smallhspace \cliverb{generate\_artifacts}    & Generate keys and permission files from a list of identities and policy files.                           \\      
\smallhspace \cliverb{generate\_policy}       & Generate XML policy file from ROS graph data.                                   \\
\smallhspace \cliverb{list\_enclaves}         & List enclaves in keystore.                                      
\end{tabularx}
%
Examples (see \href{https://github.com/ros2/sros2}{sros2 package}):
\\
\terminal{ros2 security create\_enclave demo\_keys /talker}   \\
